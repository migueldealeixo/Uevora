    ...
    while True:
        ...
        try:
            ...
            if response.get("type") == "error":
                ...
                print("Comandos: /priv, /create_group, /remove_member, /delete_group, /gmode, /sendfile, /public, /exit")
                print(f"{'='*50}\n")
...
def receive():
    ...
    while True:
        try:
            ...
            elif msg["type"] == "write_status":
                global can_write, current_writer
                can_write = msg["can_write"]
                current_writer = msg.get("current_writer")
                
                if not can_write and current_writer:
                    print(f'\n\n[Sistema] {current_writer} está a escrever... Aguarde.')
                    current_sender = None
                    # sys.stdout.write(f"\n{username}> {my_message_buffer}")
                    # sys.stdout.flush()
                elif can_write and current_writer is None:
                    print(f'\n\n[Sistema] Pode voltar a escrever.')
                    current_sender = None
                    # sys.stdout.write(f"\n{username}> {my_message_buffer}")
                    # sys.stdout.flush()
...
def send():
    ...
    try:
        ...
        while True:
            ...
            if c == '/':
                ...
                if command.startswith("priv "):
                    ...
                    try:
                        ...
                        print(f"Pedido de remoção de '{member}' do grupo '{group_name}' enviado.")
                    except ValueError:
                        print("[Erro] Uso: /remove_member <grupo> <membro>")
                    except Exception as e:
                        print(f"[Erro] {e}")
                    ...
                    try:
                        ...
                        print(f"Pedido de eliminação do grupo '{group_name}' enviado.")
                    except:
                        print("[Erro] Uso: /delete_group <nome_do_grupo>")
                    ...
                    print("  remove_member <grupo> <membro> - Remover membro (só owner)")
                    print("  delete_group <grupo>  - Apagar grupo (só owner)")
                    print("  gmode <grupo>         - Mensagem de grupo")
                    print("  sendfile <user/grupo> <ficheiro> - Enviar ficheiro")
            ...
            elif current_mode == "private" and current_recipient:
                msg_counter += 1
                private_msg = {
                    "type": "private_msg",
