...
def send_lock_status(addr, can_write, current_writer=None):
    """Informa o cliente se pode escrever ou não"""
    status_msg = {
        "type": "write_status",
        "can_write": can_write,
        "current_writer": current_writer
    }
    server_socket.sendto(json.dumps(status_msg).encode(), addr)

def handle_client(data, addr):
    global client_last_seen, writing_lock, writing_mode, writing_target
    client_last_seen[addr] = time.time()
    try:
        msg = json.loads(data.decode())
        msg_type = msg.get("type")

        if msg_type in ["char", "private_msg", "group_msg"]:
            username = clients.get(addr)
            
            # Se alguém está a escrever e não é este utilizador
            if writing_lock and writing_lock != username:
                send_lock_status(addr, False, writing_lock)
                return
            
            # Se é uma mensagem de caractere
            if msg_type == "char":
                char = msg.get("char")
                
                # Se é o primeiro caractere de uma nova mensagem
                if not writing_lock and char and ord(char) not in (10, 13, 8, 127):
                    writing_lock = username
                    writing_mode = "public"
                    # Notificar outros clientes que este utilizador começou a escrever
                    for other_addr, other_user in clients.items():
                        if other_addr != addr:
                            send_lock_status(other_addr, False, username)

                elif writing_lock and writing_lock != username:
                    send_lock_status(addr, False, writing_lock)
                    return
                
                # Se terminou a mensagem (Enter)
                elif writing_lock == username and ord(char) in (10, 13):
                    writing_lock = None
                    writing_mode = None
                    writing_target = None
                    # Notificar todos que podem voltar a escrever
                    for other_addr in clients:
                        if other_addr != addr:
                            send_lock_status(other_addr, True)
        ...
        if msg_type in ["auth", "register"]:
            ...
            if member == username:
                send_error(addr, "Não podes remover-te a ti próprio. Usa /delete_group para apagar o grupo.")
                return

            if member in groups[group]["members"]:
                groups[group]["members"].remove(member)
                save_groups()
                send_info(addr, f"{member} removido do grupo {group}.")
            ...
            del groups[group]
            save_groups()
            send_info(addr, f"Grupo '{group}' apagado.")
            ...
            if recipient in groups:
                if sender not in groups[recipient]["members"]:
                    send_error(addr, f"Não tens acesso ao grupo '{recipient}'.")
                    return
                
                if send_to_group(recipient, {
                    "type": "file_notification",
                    "message": f"{sender} está a enviar o ficheiro '{filename}' para o grupo {recipient}...",
                    "from": sender,
                    "filename": filename,
                    "group": recipient
                }, exclude_user=sender):
                    send_info(addr, f"A enviar '{filename}' para o grupo {recipient}...")
                else:
                    send_error(addr, f"Erro ao notificar o grupo '{recipient}'.")
            else:
            ...
            if recipient in groups:
                if send_to_group(recipient, {
                    "type": "file_notification", 
                    "message": f"Ficheiro '{filename}' recebido de {sender} para o grupo {recipient} e guardado na pasta ~/Downloads",
                    "from": sender,
                    "filename": filename,
                    "group": recipient
                }, exclude_user=sender):
                    print(f"[FILE] Notificação de grupo enviada: {sender} -> grupo {recipient} ({filename})")
            else:
...
def send_to_group(group_name, message, exclude_user=None):
    ...
    for member in groups[group_name]["members"]:
        if member != exclude_user:
            if send_to_user(member, message):
                sent_count += 1
